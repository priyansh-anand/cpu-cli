...
- **Clocks** come from the power manager's frequency tables in IOKit, read with `ioreg -r -d 1 -l -w0 -p IODeviceTree -n pmgr`: `voltage-states5-sram` is perflevel0, `voltage-states1-sram` is perflevel1 (only on two-perflevel chips). Each entry is a little-endian u32 frequency and a u32 voltage. M1 to M3 store hertz, the M5 stores kilohertz (4.46 GHz does not fit a u32 in hertz), so a maximum below 100 000 000 means kilohertz. Entries are not sorted, so the maximum is taken and checked to be between 100 MHz and 10 GHz; `0xffffffff` placeholder entries are skipped, and a table that is not a whole number of entries is refused. The mapping is verified only on the M5, so it is used only when perflevel1 is named `Efficiency` and the perflevel0 table is the faster one; otherwise both clocks are hidden with a diagnostic. Only the max clock is shown.
- **Rosetta 2:** an x86 build still sees every real ARM `sysctl` key, plus fake x86 ones (`machdep.cpu.family = 6`, an x86 feature list, `hw.cpufrequency = 2400000000`). The Intel path runs only when `hw.optional.arm64` is not 1 and `machdep.cpu.vendor` exists, so a translated process takes the Apple Silicon path, never reads those keys, and sets `identity.translated` from `sysctl.proc_translated`.
